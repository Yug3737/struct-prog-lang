...
def parse_simple_expression(tokens):
    ...
    if tokens[0]["tag"] == "identifier":
        identifier = tokens[0]
        tokens = tokens[1:]
        if tokens[0] == "(": # parsing a function
...
def test_parse_assignment_statement():
    ...
    tokens = tokenize("y=x+1")
    ast, _ = parse_assignment_statement(tokens)
    pprint(ast)
    assert ast == {'tag': '=',
            'target': {'position': 0, 'tag': 'identifier', 'value': 'y'},
            'value': {'left': {'position': 2, 'tag': 'identifier', 'value': 'x'},
                    'right': {'position': 4, 'tag': 'number', 'value': 1},
                    'tag': '+'}}
